...
class HealthTracker:
    ...
    def latest_checked_index(self) -> Dict[Tuple[str, str], str]:
        """(source_name, url) → latest recorded outcome timestamp."""
        return {
            (row["source_name"], row["url"]): row["last_checked_at"]
            for row in self.conn.execute(
                """
                SELECT source_name, url, MAX(recorded_at) AS last_checked_at
                FROM source_outcome_log
